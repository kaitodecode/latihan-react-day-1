...
export function DataTrainer() {
    ...
    const fetchTrainers = async () => {
        setLoading(true);
        const { data: json } = await axios.get(`${import.meta.env.VITE_API_URL}/Trainer`, {
            headers: {
    ...
    const handleEditSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await axios.put(
                `${import.meta.env.VITE_API_URL}/Trainer/${selectedTrainer.id}`,
                formData,
                {
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const handleDelete = async (id: string) => {
        if (window.confirm('Are you sure you want to delete this Trainer?')) {
            try {
                await axios.delete(`${import.meta.env.VITE_API_URL}/Trainer/${id}`, {
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json'
                    }
